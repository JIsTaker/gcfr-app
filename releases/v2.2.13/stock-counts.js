...
export function initStockCounts({supabase, getUser, toast, onAreaChange,renderBarcode,openStock}) {
  ...
  async function save(details) {
    ...
    try{
      ...
      state.pendingId=null;state.editing=null;state.selection=null;state.draftDetails=null;q('countEditor').classList.add('hidden');renderRows();toast(edit?'Count updated.':'Added to calculator.');
