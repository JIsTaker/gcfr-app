...
export function initGcfrV2Stock({
  ...
  function packageSpecSignature(row) {
    if (!row || !state.profile) return "";
    if (state.profile.stock_type === "each") {
      const units = cleanInt(row.units_per_package);
      return units > 0 ? `each|${units}|${String(row.package_label || "Carton").trim()}` : "";
    }
    const mode = row.approx_weight_mode;
    if (mode === "manual") {
      return `approx|manual|${String(row.package_label || "Carton").trim()}`;
    }
    const fixed = cleanNumber(row.fixed_package_weight_kg);
    return mode === "fixed" && fixed > 0
      ? `approx|fixed|${fixed}|${String(row.package_label || "Carton").trim()}`
      : "";
  }

  function reusableFactoryPackageTemplate(barcode) {
    const candidates = state.packages.filter((row) =>
      String(row.factory_barcode || "").trim() !== String(barcode || "").trim()
      && packageSpecSignature(row)
    );
    if (!candidates.length) return null;

    const signatures = new Set(candidates.map(packageSpecSignature));
    return signatures.size === 1 ? candidates[0] : null;
  }

  function factoryPackageEditorSeed(barcode) {
    const template = reusableFactoryPackageTemplate(barcode);
    if (!template) {
      return {
        factory_barcode: barcode,
    ...
    return {
      factory_barcode: barcode,
      package_label: template.package_label || "Carton",
      units_per_package: template.units_per_package || null,
      approx_weight_mode: template.approx_weight_mode || "fixed",
      fixed_package_weight_kg: template.fixed_package_weight_kg || null,
    };
  }
  ...
  async function continueFactoryRegistration() {
    ...
    openPackageEditor(factoryPackageEditorSeed(barcode));
  }
