...
export function createBarcodeScanner({
  ...
  shouldPauseDetection = null,
  keepAliveOnResult = false,
}) {
  let session = 0;
  let stream = null;
  let video = null;
  ...
  function pause({ hideOverlay = false, showOverlay = false } = {}) {
    if (!active) return;
    paused = true;
    if (showOverlay) {
  ...
  function resume({ showOverlay = true, rearm = false } = {}) {
    if (!active) return false;
    if (rearm) rearmCurrentScan?.();
    paused = false;
    if (showOverlay) {
      overlay.classList.remove("hidden");
      document.body.classList.add("scanner-open");
    ...
    video?.play().catch(() => {});
    status.textContent = "Ready for next barcode.";
    return true;
  }

  function shouldKeepAlive() {
    return typeof keepAliveOnResult === "function"
      ? !!keepAliveOnResult()
      : !!keepAliveOnResult;
  }
  ...
  async function start() {
    ...
    try {
      ...
      let frames = 0;
      let failures = 0;
      let confirmScan = createScanConfirmation();
      let lastDelivered = "";
      let lastDeliveredAt = 0;
      let clearFramesAfterDelivery = 0;
      rearmCurrentScan = () => {
        lastDelivered = "";
        lastDeliveredAt = 0;
        clearFramesAfterDelivery = 0;
        confirmScan = createScanConfirmation();
      };
      ...
      async function tryTicketTextRecognition() {
        ...
        try {
          ...
          status.textContent = "Reading printed code number…";
          ...
          // Barcode bars and printed-number OCR run together. Whichever one
          // wins the current SCAN ON cycle owns the result; the slower path
          // must not submit the same box a second time.
          if (processing || paused || id !== session || !active) return;
      ...
      async function scanFrame() {
        ...
        if (
          paused
          || (
            typeof shouldPauseDetection === "function"
            && shouldPauseDetection()
          )
        ) {
          timer = setTimeout(scanFrame, 80);
          return;
        }
        ...
        try {
          if (
            ...
            failures = 0;
            const rawFoundText = foundText;

            if (shouldKeepAlive() && lastDelivered) {
              if (!rawFoundText) {
                clearFramesAfterDelivery += 1;
                if (
                  clearFramesAfterDelivery >= 2
                  && Date.now() - lastDeliveredAt >= 220
                ) {
                  lastDelivered = "";
                  lastDeliveredAt = 0;
                  clearFramesAfterDelivery = 0;
              ...
              } else if (rawFoundText !== lastDelivered) {
                clearFramesAfterDelivery = 0;
              } else {
                clearFramesAfterDelivery = 0;
              }
            ...
            foundText = confirmScan(rawFoundText, frameTime, Date.now());

            if (foundText && shouldKeepAlive() && foundText === lastDelivered) {
              foundText = "";
            }

            if (foundText && (processing || paused)) {
              foundText = "";
            }

            if (foundText) {
              const delivered = foundText.trim();
              const keepAlive = shouldKeepAlive();
              processing = true;
              ...
              if (keepAlive && id === session && active) {
                status.textContent = paused
                  ? "Scan paused."
                  : "Ready for next barcode.";
                timer = setTimeout(scanFrame, 60);
              }
            ...
            if (Date.now() - started > 5500) {
              status.textContent =
                "Move closer. Barcode and printed code-number recognition are active.";
            }
