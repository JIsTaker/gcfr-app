...
export function initGcfrV2Stock({
  ...
  function canDeleteStockLinks() {
    return currentUser()?.id === "139c07f7-a826-4513-86af-25afdbe44d8f";
  }

  function showScannerProductFeedback({
    name = "-",
    code = "-",
    barcode = "-",
    total = null,
  } = {}) {
    const card = q("stockScannerDetected");
    if (!card) return;
    if (q("stockScannerDetectedName")) {
      q("stockScannerDetectedName").textContent = name || "-";
    }
    if (q("stockScannerDetectedMeta")) {
      q("stockScannerDetectedMeta").textContent =
        `Product Code ${code || "-"}`;
    }
    if (q("stockScannerDetectedResult")) {
      q("stockScannerDetectedResult").textContent =
        `Scanned: Factory Code ${barcode || "-"}`;
    }
    if (q("stockScannerDetectedTotal")) {
      q("stockScannerDetectedTotal").textContent =
        Number.isFinite(Number(total)) ? String(total) : "—";
    }
    card.classList.remove("hidden");
  }

  function clearScannerProductFeedback() {
    q("stockScannerDetected")?.classList.add("hidden");
  }

  const counts = initStockCounts({
    supabase,
    getUser: currentUser,
    toast: showToast,
    renderBarcode,
    ...
    onAreaChange(area) {
      ...
      state.backstockScanEntryMode = null;
      clearPrivateResumeCache?.();
      stopScanner();
      closeScannerList();
      closeBackstockScanMode();
      closeUnknownBarcode();
      q('stockSelectedProduct').classList.add('hidden');
    },
    onBackstockSaved(row, totalPackages) {
      showScannerProductFeedback({
        name: row?.product_name || state.selectedProduct?.name || "-",
        code: row?.product_code || state.selectedProduct?.code || "-",
        barcode: row?.details?.factoryBarcode || row?.barcode || "-",
        total: totalPackages,
      });
      setTimeout(() => {
        if (!q('screen-stock')?.classList.contains('active')) return;
        if (!state.backstockScanEntryMode) return;
        waitForNextBackstockScan();
      }, 60);
    },
    onBackstockCancelled() {
      state.selectedProduct = null;
      state.profile = null;
      state.packages = [];
      state.selectedPackageId = "";
      q("stockSelectedProduct")?.classList.add("hidden");
      clearScannerProductFeedback();
      clearPrivateResumeCache?.();
      if (!q('screen-stock')?.classList.contains('active')) return;
      if (!state.backstockScanEntryMode) return;
      waitForNextBackstockScan();
  ...
  function renderSearchResults(matches) {
    ...
    for (const product of matches) {
      ...
      const searchMatch = product.__stockSearchMatch || null;
      button.innerHTML = `
        <span>
          <strong>${escapeHtml(product.name)}</strong>
          <small>${searchMatch?.type === "factory"
            ? `Factory Code ${escapeHtml(searchMatch.value)} · Product Code ${escapeHtml(product.code)}`
            : `Product Code ${escapeHtml(product.code)}`}</small>
        </span>
        <b>Open</b>
      `;
      button.onclick = () => {
        ...
        void selectProduct(product, {
          factory: searchMatch.factory || null,
          barcode: searchMatch.value || "",
          deferCount: true,
        });
      };
  ...
  async function searchProducts(rawQuery) {
    ...
    try {
      ...
      const ranked = rankProducts(catalog, query);
      const barcode = normalizeScannedBarcode(query);

      if (barcode && !/\s/.test(query)) {
        const factoryMatch = await findFactoryCodeLookup(barcode);
        const product = factoryMatch?.product
          || (factoryMatch?.product_code
            ? catalog.find((row) => String(row.code) === String(factoryMatch.product_code))
            : null);
        ...
        if (product) {
          const matched = {
            ...
            __stockSearchMatch: {
              type: "factory",
              value: barcode,
              factory: factoryMatch.factory || null,
            },
          };
          renderSearchResults([
            matched,
            ...ranked.filter((row) => String(row.code) !== String(product.code)),
          ]);
          return;
        }
      }
  ...
  async function selectProduct(product, options = {}) {
    ...
    try {
      ...
      if (options.factory && !options.deferPackageChoice) {
        state.selectedPackageId = options.factory.id;
      }
      ...
      if (!options.deferCount) {
        await counts.select({
          product: state.selectedProduct,
          profile: state.profile,
          pack: options.factory || selectedPackage(),
          barcode,
        }, {
          auto: !!options.auto,
          backstockMode: options.backstockMode || state.backstockScanEntryMode || "quantity",
        });
      }
    } catch (error) {
  ...
  function stockLinkLabel(type) {
    return type === "ticket_barcode"
      ? "Store Ticket"
      : type === "factory_barcode"
        ? "Factory Code"
        : "Selling Barcode";
  }

  function setupRowLinks(row) {
    const links = [];
    const seen = new Set();

    for (const item of row.barcodes || []) {
      const barcode = String(item.barcode || "").trim();
      const type = item.barcode_type || "product_code";
      if (!barcode) continue;
      const key = `${type}|${barcode}`;
      if (seen.has(key)) continue;
      seen.add(key);
      links.push({ barcode, barcode_type: type });
    }

    for (const pack of row.packages || []) {
      const barcode = String(pack.factory_barcode || "").trim();
      if (!barcode) continue;
      const key = `factory_barcode|${barcode}`;
      if (seen.has(key)) continue;
      seen.add(key);
      links.push({ barcode, barcode_type: "factory_barcode" });
    }

    return links;
  }

  async function unlinkStockBarcode({
    barcode,
    barcodeType,
    productCode,
    productName,
    ...
    if (!canDeleteStockLinks()) return;

    const code = String(productCode || "").trim();
    const value = String(barcode || "").trim();
    const type = String(barcodeType || "").trim();
    if (!code || !value || !type) return;

    const label = stockLinkLabel(type);
    if (!confirm(`Unlink ${label} ${value} from ${productName || code}?`)) return;
    ...
    const { error } = await supabase.rpc("gcfr_unlink_stock_barcode", {
      _barcode: value,
      _product_code: code,
      _barcode_type: type,
    });
    ...
    if (button) {
      ...
      button.textContent = previousText || "Unlink";
    }
    ...
    showToast(`${label} unlinked.`);

    if (state.selectedProduct?.code === code) {
      await loadProfileAndPackages(false);
      renderBarcodeLinks();
      renderPackages();
  ...
  function renderSetupData() {
    ...
    for (const row of rows) {
      ...
      card.innerHTML = `
        ...
        <div class="stock-data-link-list"></div>
        <div class="stock-package-actions"><button type="button" class="secondary stock-data-edit-btn">Edit / Register</button></div>
      `;

      const linkList = card.querySelector(".stock-data-link-list");
      if (!links.length) {
        linkList.innerHTML = '<small class="stock-data-no-links">No linked barcode</small>';
      } else {
        for (const link of links) {
          const item = document.createElement("div");
          item.className = "stock-data-link-row";
          item.innerHTML = `
            <span>
              <small>${escapeHtml(stockLinkLabel(link.barcode_type))}</small>
              <strong>${escapeHtml(link.barcode)}</strong>
            </span>
          `;

          if (canDeleteStockLinks()) {
            const unlink = document.createElement("button");
            unlink.type = "button";
            unlink.className = "text-button danger stock-data-unlink-btn";
            unlink.textContent = "Unlink";
            unlink.onclick = (event) => {
              event.preventDefault();
              event.stopPropagation();
              void unlinkStockBarcode({
                barcode: link.barcode,
                barcodeType: link.barcode_type,
      ...
      card.querySelector(".stock-data-edit-btn").onclick = () => {
        closeUnknownBarcode();
  ...
  function clearAdminRegistrationSearch() {
    for (const id of ["adminStockSearch", "adminStockLinkSearch"]) {
      const input = q(id);
      if (input) input.value = "";
    }
    for (const id of ["adminStockSearchResults", "adminStockLinkSearchResults"]) {
      const box = q(id);
      if (!box) continue;
      box.innerHTML = "";
      box.classList.add("hidden");
  ...
  function focusAdminRegistrationStart(message = "Step 1 · Scan the barcode to register.") {
    if (q("adminStockStep")) q("adminStockStep").textContent = message;
    const button = q("adminStockScanBtn");
    const anchor = button?.closest(".admin-stock-scan-row") || button;
  ...
  function clearAdminSelectedProduct() {
    state.selectedProduct = null;
    state.profile = null;
    state.packages = [];
    state.barcodeLinks = [];
    state.selectedPackageId = "";
    q("adminStockSelected")?.classList.add("hidden");
    q("stockSetupPanel")?.classList.add("hidden");
    closePackageEditor();
  }

  function prepareAdminRegistrationStart() {
    closeUnknownBarcode();
    clearAdminRegistrationSearch();
    clearAdminSelectedProduct();
    state.adminScanPurpose = "register";
  }

  function finishAdminRegistration(message) {
    closeUnknownBarcode();
    clearAdminRegistrationSearch();
    clearAdminSelectedProduct();
    state.adminScanPurpose = "register";
    focusAdminRegistrationStart(message);
  }

  async function continueFactoryRegistration() {
    const barcode = state.pendingBarcode;
    if (state.pendingBarcodeMode !== "factory" || !barcode || !state.selectedProduct) return;

    if (!state.profile) {
      openProfileSetup(false);
      focusStep("stockTypeSelect", "Factory Code linked · Set product type once.");
      return;
    }

    openPackageEditor({ factory_barcode: barcode, package_label: "Carton" });
  }

  async function selectForAdmin(product) {
    closePackageEditor();
    const pendingMode = state.pendingBarcodeMode;
    ...
    if (["selling", "ticket"].includes(pendingMode) && pendingBarcode) {
      if (await savePendingProductBarcode()) {
        finishAdminRegistration("Saved · Scan the next barcode.");
      }
      return;
    }

    if (pendingMode === "factory" && pendingBarcode) {
      const { error: linkError } = await supabase.rpc("gcfr_link_stock_barcode", {
        _barcode: pendingBarcode,
      ...
      await continueFactoryRegistration();
      return;
    }
  ...
  function renderBarcodeLinks() {
    ...
    for (const row of rows) {
      ...
      if (canDeleteStockLinks()) {
        const unlink = document.createElement("button");
        unlink.type = "button";
        unlink.className = "text-button danger";
        unlink.textContent = "Unlink";
        unlink.onclick = () => void unlinkStockBarcode({
          barcode: row.barcode,
        ...
        card.querySelector(".stock-package-actions").appendChild(unlink);
      }
  ...
  function openPackageEditor(row = null) {
    ...
    q("stockPackageLabel").value =
      source.package_label || (source.factory_barcode || state.pendingBarcode ? "Carton" : "");
    ...
    const targetId = state.profile.stock_type === "each"
      ? "stockUnitsPerPackage"
      : "stockApproxWeightMode";
    if (q("adminStockStep")) {
      q("adminStockStep").textContent = "Step 3 · Confirm carton quantity / weight and save.";
    }
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        q("stockPackagePanel")?.scrollIntoView({ behavior: "auto", block: "start" });
        q(targetId)?.focus({ preventScroll: true });
      });
    });
  }
  ...
  async function savePackage(event) {
    ...
    showToast("Factory Code + carton saved.");
    await loadProfileAndPackages(false);
    renderPackages();
    renderBarcodeLinks();
    void refreshSetupData();
    ...
    renderCalculator();
    finishAdminRegistration("Saved · Scan the next Factory Code.");
  }
  ...
  function setScanMode(mode) {
    ...
    stopScanner();
    q("stockScannerListPanel")?.classList.add("hidden");
    q("stockScanModeModal")?.classList.add("hidden");
    q("stockScannerScanOnBtn")?.classList.add("hidden");
    state.backstockScanEntryMode = null;
  ...
  function shouldKeepBackstockCameraAlive() {
    return (
      state.stockScanPurpose === "count"
      && state.scanMode === "backstock"
      && !!state.backstockScanEntryMode
    );
  }

  function hideScannerScanOn() {
    q("stockScannerScanOnBtn")?.classList.add("hidden");
  }

  function waitForNextBackstockScan() {
    const scanner = getScanner();
    if (!scanner.isActive?.()) return;
    scanner.pause?.({ showOverlay: true });
    q("stockScanStatus").textContent =
      "Scan complete · press SCAN ON for the next box.";
    q("stockScannerScanOnBtn")?.classList.remove("hidden");
  }

  function armNextBackstockScan() {
    if (!shouldKeepBackstockCameraAlive()) return;
    const scanner = getScanner();
    hideScannerScanOn();
    if (scanner.isActive?.()) {
      scanner.resume?.({ showOverlay: true, rearm: true });
      q("stockScanStatus").textContent = "Scanning…";
      return;
    }
    void scanner.start();
  }

  function resumeBackstockScanner() {
    const scanner = getScanner();
    hideScannerScanOn();
    if (scanner.isActive?.()) {
      scanner.resume?.({ showOverlay: true });
      return;
    }
    void scanner.start();
  }
    ...
    if (!state.scanner) {
      state.scanner = createBarcodeScanner({
        ...
        onResult: async (raw) => {
          ...
          const keepAlive = shouldKeepBackstockCameraAlive();
          if (!keepAlive) {
            try {
              state.scanner?.stop();
            } catch {}
        ...
        // Keep the printed-number fallback active for both Backstock and
        // Shopfloor stock counting. Backstock factory labels often have a
        // readable numeric code even when the bars are damaged or blurred.
        shouldUseTextRecognition: () => state.stockScanPurpose === "count",
        shouldPauseDetection: () =>
          !q("stockScannerListPanel")?.classList.contains("hidden"),
        keepAliveOnResult: () => shouldKeepBackstockCameraAlive(),
        onError: (error) => {
  ...
  function closeBackstockScanMode() {
    q("stockScanModeModal")?.classList.add("hidden");
  }

  function chooseBackstockScanMode(mode) {
    state.backstockScanEntryMode = mode === "quantity" ? "quantity" : "auto";
    clearScannerProductFeedback();
    hideScannerScanOn();
    closeBackstockScanMode();
    void startScannerDirect();
  }
  ...
  function startScanner() {
    if (state.scanMode !== "backstock") {
      state.backstockScanEntryMode = null;
      clearScannerProductFeedback();
      return startScannerDirect();
    }

    state.backstockScanEntryMode = null;
    q("stockScanModeModal")?.classList.remove("hidden");
    return undefined;
  }

  function openScannerList() {
    const panel = q("stockScannerListPanel");
    const body = q("stockScannerListBody");
    if (!panel || !body) return;

    panel.classList.remove("hidden");
    body.innerHTML =
      '<div class="empty-state scanner-list-empty">No Backstock scanned yet. Scan a Factory Code to add stock.</div>';

    try {
      counts.renderBackstockList?.(body);
    } catch (error) {
      console.error("Backstock scanner list:", error);
      body.innerHTML =
        '<div class="empty-state scanner-list-empty">Could not load the list. Close this popup and try again.</div>';
    }
  }

  function closeScannerList() {
    q("stockScannerListPanel")?.classList.add("hidden");
  }
  ...
  async function findFactoryCodeLookup(barcode, { resolveProduct = false } = {}) {
    const factory = await findFactoryBarcode(barcode);
    if (factory) {
      return {
        factory,
        product_code: factory.product_code,
        product: resolveProduct ? await findProductByCode(factory.product_code) : null,
        source: "factory_package",
      };
    }

    const stored = await findStoredBarcode(barcode);
    if (stored?.barcode_type !== "factory_barcode") return null;

    return {
      factory: null,
      product_code: stored.product?.code || "",
  ...
  async function resolveTicketTextCandidate(candidates, adminMode = false, meta = {}) {
    ...
    for (const value of values) {
      ...
      const factoryMatch = await findFactoryCodeLookup(value);
      if (factoryMatch && (adminMode || state.scanMode === "backstock")) {
        return { value, known: true };
      }

      const stored = factoryMatch ? null : await findStoredBarcode(value);
      if (!stored) continue;
  ...
  async function handleBarcode(rawBarcode) {
    ...
    try {
      const factoryMatch = await findFactoryCodeLookup(barcode, { resolveProduct: true });
      if(request!==state.scanRequest)return;

      if (state.scanMode === "shopfloor" && factoryMatch) {
        showToast("Factory crate/carton barcode belongs to Backstock Scanning.");
        return;
      }

      if (factoryMatch) {
        if (counts.area() === 'shopfloor') {
          showToast('Use the selling barcode or product ticket for shopfloor.');
          return;
        }

        const product = factoryMatch.product
          || await findProductByCode(factoryMatch.product_code);
        if(request!==state.scanRequest)return;

        if (product) {
          const factory = factoryMatch.factory;
          ...
          if (!factory) {
            ...
            waitForNextBackstockScan();
            showToast(`Factory Code found: ${product.name}. Carton quantity/weight setup is missing.`, 5000);
            return;
          }

          if (state.backstockScanEntryMode === "quantity") {
            // Keep the live camera visible behind the quantity popup.
            // Only barcode detection pauses until the quantity is saved.
            state.scanner?.pause?.({ showOverlay: true });
            hideScannerScanOn();
            if (q("stockScanStatus")) {
              q("stockScanStatus").textContent = "Enter quantity for this carton.";
            }
          }
          ...
          await selectProduct(product, {
            ...
            barcode,
            auto: true,
            backstockMode: state.backstockScanEntryMode || "quantity",
          });
          return;
        }
      }
      ...
      if(counts.area()==='backstock') {
        // Unknown Factory Codes leave the camera workflow and enter registration.
        // Stop the MediaStream completely instead of only hiding the overlay.
        state.scanner?.stop?.();
        hideScannerScanOn();
  ...
  function startAdminStockScanner(purpose = "register") {
    const resolvedPurpose = typeof purpose === "string" ? purpose : "register";
    if (resolvedPurpose === "register") {
      prepareAdminRegistrationStart();
    }
    state.adminScanPurpose = resolvedPurpose;
  ...
  const privateResumeFieldIds = [
    "adminStockRegistrationMode",
    "adminStockSearch",
    "adminStockLinkSearch",
    "stockProductSearch",
    "stockTypeSelect",
    "stockDefaultUnitWeight",
    "stockFactoryBarcode",
    "stockPackageLabel",
    "stockUnitsPerPackage",
    "stockApproxWeightMode",
    "stockFixedPackageWeight",
    "stockPackageEditId",
    "stockNewPackagesEach",
    "stockRemainingEach",
    "stockNewPackagesApprox",
    ...
    "stockSampleWeight2",
    "stockSampleWeight3",
    "stockUseProductWeight",
    "stockUseManualWeight",
  ];

  function capturePrivateResumeFields() {
    const fields = {};
    for (const id of privateResumeFieldIds) {
      const element = q(id);
      if (!element) continue;
      if (element.type === "checkbox" || element.type === "radio") {
        fields[id] = { checked: !!element.checked };
      } else {
        fields[id] = { value: element.value };
      }
    }
    return fields;
  }

  function applyPrivateResumeFields(fields = {}) {
    for (const [id, saved] of Object.entries(fields)) {
      const element = q(id);
      if (!element || !saved) continue;
      if ("checked" in saved) element.checked = !!saved.checked;
      if ("value" in saved) element.value = saved.value ?? "";
    }
    syncProfileSetupFields();
    syncPackageEditorFields();
    syncWeightMode();
  ...
  function snapshotPrivateWork() {
    ...
    const countWork = counts.snapshotPrivateWork?.() || null;
    const adminSetupOpen = !q("adminStockSetupView")?.classList.contains("hidden");
    const stockActive = q("screen-stock")?.classList.contains("active");
    const hasDraft =
      !!countWork
      || !!state.pendingBarcode
      || !!state.selectedProduct
      || !q("stockPackagePanel")?.classList.contains("hidden")
      || !q("stockSetupPanel")?.classList.contains("hidden");

    if (!hasDraft && !adminSetupOpen && !stockActive) return null;

    return {
      userId: current.id,
      context: adminSetupOpen ? "admin-stock-setup" : (stockActive ? "stock" : "background"),
      selectedProduct: state.selectedProduct ? structuredClone(state.selectedProduct) : null,
      selectedPackageId: state.selectedPackageId || "",
      pendingBarcode: state.pendingBarcode || "",
      pendingBarcodeMode: state.pendingBarcodeMode || "",
      adminScanPurpose: state.adminScanPurpose || "register",
      scanMode: state.scanMode || "backstock",
      weightMode: state.weightMode || "product",
      backstockEntryMode: state.backstockEntryMode || "used",
      activeShopfloorLayers: [...state.activeShopfloorLayers],
      manualWeights: [...state.manualWeights],
      packageOpen: !q("stockPackagePanel")?.classList.contains("hidden"),
      profileOpen: !q("stockSetupPanel")?.classList.contains("hidden"),
      unknownOpen: !q("adminStockScanUnknownPanel")?.classList.contains("hidden"),
      fields: capturePrivateResumeFields(),
      countWork,
    };
  }

  async function restorePrivateWork(snapshot) {
    const current = currentUser();
    if (!current || !snapshot || snapshot.userId !== current.id) return false;

    state.pendingBarcode = snapshot.pendingBarcode || "";
    state.pendingBarcodeMode = snapshot.pendingBarcodeMode || "";
    state.adminScanPurpose = snapshot.adminScanPurpose || "register";
    state.scanMode = snapshot.scanMode === "shopfloor" ? "shopfloor" : "backstock";
    state.weightMode = snapshot.weightMode === "manual" ? "manual" : "product";
    state.backstockEntryMode = snapshot.backstockEntryMode || "used";
    state.activeShopfloorLayers = new Set(
      Array.isArray(snapshot.activeShopfloorLayers) ? snapshot.activeShopfloorLayers : []
    );
    state.manualWeights = Array.isArray(snapshot.manualWeights)
      ? snapshot.manualWeights.map((value) => Number(value)).filter(Number.isFinite)
      : [];

    const backstock = state.scanMode === "backstock";
    if (q("stockBackstockModeInput")) q("stockBackstockModeInput").checked = backstock;
    if (q("stockShopfloorModeInput")) q("stockShopfloorModeInput").checked = !backstock;
    q("stockBackstockModeInput")?.closest(".stock-mode-btn")?.classList.toggle("active", backstock);
    q("stockShopfloorModeInput")?.closest(".stock-mode-btn")?.classList.toggle("active", !backstock);

    if (snapshot.context === "admin-stock-setup") {
      q("adminHomeView")?.classList.add("hidden");
      q("adminBarcodeDetailView")?.classList.add("hidden");
      q("adminProcessPendingView")?.classList.add("hidden");
      q("adminPricingView")?.classList.add("hidden");
      q("adminStockSetupView")?.classList.remove("hidden");
    }

    if (snapshot.selectedProduct?.code) {
      state.selectedProduct = {
        code: String(snapshot.selectedProduct.code),
        name: String(snapshot.selectedProduct.name || snapshot.selectedProduct.code),
      };
      state.selectedPackageId = snapshot.selectedPackageId || "";
      state.profile = null;
      state.packages = [];
      state.barcodeLinks = [];
      await loadProfileAndPackages(false);
      ...
      if (snapshot.context === "admin-stock-setup") {
        if (q("adminStockSelectedName")) q("adminStockSelectedName").textContent = state.selectedProduct.name;
        if (q("adminStockSelectedCode")) q("adminStockSelectedCode").textContent = state.selectedProduct.code;
        q("adminStockSelected")?.classList.remove("hidden");
        q("stockSetupBtn")?.classList.remove("hidden");
        q("stockAddPackageBtn")?.classList.toggle("hidden", !state.profile);
        renderProfileSummary();
        ...
        if (snapshot.profileOpen) openProfileSetup(false);
        if (snapshot.packageOpen && state.profile) {
          openPackageEditor({
            factory_barcode: state.pendingBarcode || snapshot.fields?.stockFactoryBarcode?.value || "",
            package_label: "Carton",
          });
        }
      } else if (snapshot.context === "stock") {
        q("stockSelectedProduct")?.classList.remove("hidden");
        if (q("stockSelectedName")) q("stockSelectedName").textContent = state.selectedProduct.name;
        if (q("stockSelectedCode")) q("stockSelectedCode").textContent = state.selectedProduct.code;
        renderSelectedProduct();
      }
    }

    if (
      snapshot.context === "admin-stock-setup"
      && snapshot.unknownOpen
      && state.pendingBarcode
    ) {
      if (q("adminStockScanUnknownBarcode")) {
        q("adminStockScanUnknownBarcode").textContent = state.pendingBarcode;
      }
      if (q("adminStockScanUnknownMessage")) {
        q("adminStockScanUnknownMessage").textContent =
          "Registration restored after update. Continue linking this barcode.";
      }
      q("adminStockScanUnknownPanel")?.classList.remove("hidden");
    } else {
      q("adminStockScanUnknownPanel")?.classList.add("hidden");
    ...
    applyPrivateResumeFields(snapshot.fields || {});
    if (snapshot.countWork) counts.restorePrivateWork?.(snapshot.countWork);
    return true;
  }
  ...
  function onScreenChange(screen) {
    ...
    if (screen !== "stock") {
      const discarded = counts.discardPendingEntry?.() || false;
      if (discarded) {
        state.selectedProduct = null;
        state.profile = null;
        state.packages = [];
  ...
  q("stockScanModeNoQtyBtn")?.addEventListener("click", () => chooseBackstockScanMode("auto"));
  q("stockScanModeQtyBtn")?.addEventListener("click", () => chooseBackstockScanMode("quantity"));
  q("stockScanModeCloseBtn")?.addEventListener("click", closeBackstockScanMode);
  q("stockScanModeBackdrop")?.addEventListener("click", closeBackstockScanMode);
  q("stockScannerListBtn")?.addEventListener("click", openScannerList);
  q("stockScannerListCloseBtn")?.addEventListener("click", closeScannerList);
  q("stockScannerScanOnBtn")?.addEventListener("click", armNextBackstockScan);
  ...
  q("stockUnknownCloseBtn").onclick = () => {
    closeUnknownBarcode();
    if (shouldKeepBackstockCameraAlive()) resumeBackstockScanner();
  };
  ...
  q("adminStockRegistrationMode")?.addEventListener("change", () => {
    prepareAdminRegistrationStart();
    focusAdminRegistrationStart("Step 1 · Scan the barcode to register.");
  });
