...
function loadNotificationSettings() {
  try {
    ...
    return {
      ...
      system: stored.system ?? DEFAULT_NOTIFICATION_SETTINGS.system,
    };
  } catch {
...
function handleRunItemRealtime(payload) {
  ...
  if (row.status === "done") {
    fireGcfrNotification({
      key: `run-item-done-${row.id}-${row.completed_at || ""}`,
    ...
    fireGcfrNotification({
      key: `run-item-oos-${row.id}-${row.completed_at || ""}`,
      title: "Out of stock",
      body: row.manual_name || row.product_code
...
function handleChecklistRealtime(payload) {
  ...
  if (
    ...
    fireGcfrNotification({
      key: `checklist-done-${row.id}-${row.completed_at || ""}`,
...
function updateNotificationPermissionUi() {
  ...
  const button = $("enableSystemNotificationsBtn");
  ...
  if (permission === "granted") {
    status.textContent = "Enabled for this device.";
    button.textContent = "Enabled";
    button.disabled = true;
  } else if (permission === "denied") {
    status.textContent = "Blocked in browser/device settings.";
    button.textContent = "Blocked";
    button.disabled = true;
  } else {
    status.textContent = "Tap Enable to allow system notification banners.";
...
async function requestSystemNotifications() {
  if (!("Notification" in window)) {
    showToast("System notifications are not supported on this browser.");
    return;
  }
  ...
  try {
    ...
    if (permission === "granted") {
      ...
      showToast("System notification permission was not granted.");
    }
  } catch (error) {
    showToast(error.message || "Could not request notification permission.");
  }
}
...
function subscribeRealtime() {
  ...
  const runChannel = supabase
    .channel("gcfr-runs")
    .on("postgres_changes", { event: "*", schema: "public", table: "run_lists" }, (payload) => {
      handleRunListRealtime(payload);
      refreshRuns();
      refreshHistory();
    })
    .on("postgres_changes", { event: "*", schema: "public", table: "run_items" }, (payload) => {
      handleRunItemRealtime(payload);
      refreshRuns();
      refreshHistory();
    })
    .subscribe();
  ...
  const checklistChannel = supabase
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "checklist_tasks" }, (payload) => {
      handleChecklistRealtime(payload);
...
$("enableSystemNotificationsBtn").onclick = requestSystemNotifications;
