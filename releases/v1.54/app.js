...
function updateRunBulkDeleteControls() {
  ...
  tools.classList.toggle("hidden", !isAdmin || !hasRuns);
...
$("runBulkDeleteConfirmBtn").onclick = async () => {
  if (currentUser?.id !== ADMIN_USER_ID || !runBulkDeleteIds.size) return;
...
async function refreshRuns() {
  ...
  for (const run of visibleRuns) {
    ...
    if (runBulkDeleteMode && currentUser?.id === ADMIN_USER_ID) {
    ...
    if (currentUser?.id === ADMIN_USER_ID && !runBulkDeleteMode) {
      ...
      deleteRun.onclick = async () => {
        if (!confirm(`Delete Run #${runNumber}?`)) return;
        ...
        showToast(`Run #${runNumber} moved to History.`);
        await refreshRuns();
        await refreshHistory();
      };
...
async function refreshChecklist() {
  ...
  currentChecklistTaskIds = (data || []).map((task) => task.id);
