...
async function chooseRearCamera() {
  try {
    ...
    return rear?.id || { facingMode: "environment" };
  } catch {
    return { facingMode: "environment" };
  }
}
...
async function startScanner() {
  ...
  try {
    ...
    scanner = new Html5Qrcode(
      "reader",
      formatsToSupport?.length
        ? { formatsToSupport, verbose: false }
        : { verbose: false },
    );
    ...
    await scanner.start(
      camera,
      {
        ...
        qrbox: (viewfinderWidth, viewfinderHeight) => {
          ...
          const height = Math.floor(
            Math.min(viewfinderHeight * 0.34, 170)
          );
      ...
      async (decodedText) => {
        ...
        try {
          await handleScannedBarcode(value);
        } catch (error) {
          console.error(error);
    ...
    setScanStatus("Point the rear camera at the barcode.");
    scanBusy = false;
  } catch (error) {
    ...
    if (/permission|notallowed|denied/i.test(message)) {
      showToast("Camera permission is blocked. Allow Camera for this site and try again.");
    } else if (/notfound|camera/i.test(message)) {
      showToast("No usable camera was found.");
    } else {
      showToast("Barcode scanner could not start. Check camera permission and connection.");
    }
  }
}

async function stopScanner({ preserveBusy = false } = {}) {
  if (!preserveBusy) scanBusy = true;
