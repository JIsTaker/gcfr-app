...
$("rosterDatePicker").onchange = async () => {
  const value = $("rosterDatePicker").value;
  if (!value) return;

  rosterSelectedDate = value;
  rosterInitialScrollDone = false;
  syncRosterDateControls();
  closeRosterShiftForm();
  await refreshRoster();
...
$("rosterShiftForm").onsubmit = async (event) => {
  ...
  if (!userId || !startTime || !endTime) {
    showToast("Team Member, Start and Finish are required.");
    return;
  }
  ...
  closeRosterShiftForm();
  showToast(shiftId ? "Shift updated." : "Shift added.");
  await refreshRoster();
};
...
function syncRosterDateControls() {
  const picker = $("rosterDatePicker");
  const display = $("rosterDateDisplay");

  if (picker) picker.value = rosterSelectedDate;
...
function renderRosterScheduler() {
  ...
  for (const userId of memberIds) {
    ...
    name.textContent = rosterProfileMap.get(userId) || "User";
    ...
    for (const shift of grouped.get(userId) || []) {
  ...
  if (!rosterInitialScrollDone) {
    const earliest = shifts.reduce((min, shift) => {
      const start = rosterTimeToMinutes(shift.start_time);
      return Math.min(min, start);
    }, 7 * 60);

    const targetHour = Math.max(0, Math.floor(earliest / 60) - 1);
    requestAnimationFrame(() => {
      scroll.scrollLeft = targetHour * 52;
    });
...
async function refreshBarcodeData() {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .select("barcode, product_code, updated_at, products(name)")
