...
async function loadProductLookupPrice(row) {
  ...
  try {
    ...
    const hasWas = Number.isFinite(was) && was > 0;
    const promo = Boolean(
      product.is_promo
      || (hasNow && hasWas && was > now)
    );

    const priceHtml = promo && hasNow && hasWas && was > now
      ? `
          <span class="lookup-price-original">${formatAud(was)}</span>
          <strong class="lookup-price-current promo">${formatAud(now)}</strong>
        `
      : `
          <strong class="lookup-price-current">${hasNow ? formatAud(now) : "Price unavailable"}</strong>
        `;
    ...
    panel.innerHTML = `
      ...
      <div class="lookup-price-values">${priceHtml}</div>
      ${promoText ? `<div class="lookup-price-promo-text">${escapeHtml(promoText)}</div>` : ""}
      ${product.comparable ? `<div class="lookup-price-unit">${escapeHtml(product.comparable)}</div>` : ""}
    `;
  } catch (error) {
...
async function refreshRuns() {
  ...
  headInfo.innerHTML = `
    ...
    <div class="item-sub">${allItems.length} item${allItems.length === 1 ? "" : "s"} · A–Z</div>
  `;
