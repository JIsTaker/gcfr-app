...
import { createMyShiftController } from "./my-shift.js";
import { createFreshResetController } from "./fresh-reset.js";
...
const PRIVATE_RESUME_PREFIX = "gcfr_private_resume_v1_";
const PRIVATE_RESUME_MAX_AGE_MS = 12 * 60 * 60 * 1000;
...
let myShiftController = null;
let freshResetController = null;
...
let rosterDayOffUserIds = new Set();
let rosterWeekOffKeys = new Set();
let selectedRosterShiftDetail = null;
...
function privateResumeKey(userId = currentUser?.id) {
  return userId ? `${PRIVATE_RESUME_PREFIX}${userId}` : "";
}

function clearPrivateResumeCache() {
  const key = privateResumeKey();
  if (!key) return;
  try {
...
function snapshotPrivateWorkBeforeForcedUpdate(event) {
  if (!currentUser?.id) return;
  const key = privateResumeKey(currentUser.id);
  if (!key) return;

  const activeScreen = document.querySelector(".screen.active")?.id?.replace(/^screen-/, "") || "myshift";
  const payload = {
    schema: 1,
    userId: currentUser.id,
    savedAt: Date.now(),
    fromRelease: event?.detail?.from || null,
    toRelease: event?.detail?.to || null,
    screen: activeScreen,
    scrollX: window.scrollX || 0,
    scrollY: window.scrollY || 0,
    fields: capturePrivateResumeFields(),
    stock: stockController?.snapshotPrivateWork?.() || null,
  };

  try {
    sessionStorage.setItem(key, JSON.stringify(payload));
  } catch (error) {
    console.warn("Could not cache private work for update:", error);
  }
}

async function restorePrivateWorkAfterForcedUpdate(isAdmin) {
  ...
  if (
    !snapshot
    || snapshot.userId !== currentUser.id
    || !Number.isFinite(Number(snapshot.savedAt))
    || Date.now() - Number(snapshot.savedAt) > PRIVATE_RESUME_MAX_AGE_MS
  ) {
    sessionStorage.removeItem(key);
    return false;
  }
  ...
  try {
    let screen = String(snapshot.screen || "myshift");
    const stockSetupResume = snapshot.stock?.context === "admin-stock-setup";

    if (screen === "admin" && stockSetupResume && canManageProductData()) {
      limitedStockSetupEntry = !isAdmin;
    ...
    if (snapshot.stock) {
      await stockController?.restorePrivateWork?.(snapshot.stock);
    }
    ...
    sessionStorage.removeItem(key);
    showToast("Update complete · your unfinished work was restored.", 3600);
    return true;
  } catch (error) {
    console.warn("Could not restore private work after update:", error);
    sessionStorage.removeItem(key);
    return false;
  }
}

window.addEventListener("gcfr:before-forced-update", snapshotPrivateWorkBeforeForcedUpdate);
...
async function enterApp() {
  ...
  // My Shift is the default work screen. A saved in-progress screen is restored below.
  document.querySelectorAll(".screen").forEach((el) => el.classList.remove("active"));
  $("screen-myshift").classList.add("active");
  document.querySelectorAll(".bottom-nav button[data-screen]").forEach((b) => b.classList.remove("active"));
  document.querySelector('.bottom-nav button[data-screen="myshift"]')?.classList.add("active");
  $("screenTitle").textContent = "My Shift";
  ...
  [
    "openOwnerDocumentsBtn",
    "openFreshResetBtn",
    "adminOwnerDocumentsView",
    "adminFreshResetView",
  ].forEach((id) => $(id)?.classList.toggle("hidden", !isAdmin));
  ...
  subscribeRealtime();
  await myShiftController?.start();

  const resumedAfterUpdate = await restorePrivateWorkAfterForcedUpdate(isAdmin);
  if (!resumedAfterUpdate) {
    const savedScreen = localStorage.getItem(`gcfr_last_screen_${currentUser.id}`);
    if (savedScreen && savedScreen !== "myshift" && (savedScreen !== "admin" || isAdmin)) {
...
async function leaveApp() {
  ...
  myShiftController?.reset();
  freshResetController?.reset();
...
function resetAppTransientUiForAccountChange() {
  ...
  rosterDayShifts = [];
  rosterWeekShifts = [];
  rosterDayOffUserIds = new Set();
  rosterWeekOffKeys = new Set();
  selectedRosterShiftDetail = null;
...
freshResetController = createFreshResetController({
  supabase,
  $,
  showToast,
  getCurrentUser: () => currentUser,
...
myShiftController = createMyShiftController({
  supabase,
  $,
  showToast,
  getCurrentUser: () => currentUser,
  ...
  localDateString,
  formatDateTime,
  getRosterSelectedDate: () => rosterSelectedDate,
});
...
const moreScreens = new Set(["runs", "history", "roster", "admin", "account"]);
...
async function navigateToScreen(screen) {
  ...
  if (screen !== "roster") closeRosterShiftDetail();
  stockController?.onScreenChange(screen);

  if (screen === "myshift") await myShiftController?.refresh();
...
$("rosterShiftDetailCloseBtn").onclick = closeRosterShiftDetail;
$("rosterShiftDetailBackdrop").onclick = closeRosterShiftDetail;
$("rosterShiftDetailOffBtn").onclick = toggleSelectedRosterOff;
$("rosterShiftDetailEditBtn").onclick = async () => {
  if (!isOwnerUser() || !selectedRosterShiftDetail) return;
  const shift = selectedRosterShiftDetail;
  const wasWeekView = rosterWeekOverviewOpen;
  closeRosterShiftDetail();
...
async function refreshRoster() {
  ...
  try {
    ...
    const [shiftResult, offResult] = await Promise.all([
      supabase
        .from("gcfr_roster_shifts")
      ...
      supabase
        .from("gcfr_roster_off_days")
        .select("user_id")
        .eq("off_date", rosterSelectedDate),
    ]);

    if (shiftResult.error) throw shiftResult.error;
    if (offResult.error) throw offResult.error;

    rosterDayShifts = shiftResult.data || [];
    rosterDayOffUserIds = new Set((offResult.data || []).map((row) => row.user_id));
    renderRosterScheduler();
  } catch (error) {
...
function renderRosterScheduler() {
  ...
  for (const userId of memberIds) {
    ...
    if (rosterDayOffUserIds.has(userId)) {
      const offBadge = document.createElement("span");
      offBadge.className = "roster-member-off-badge";
      offBadge.textContent = "OFF";
      name.append(memberName, memberTime, offBadge);
    } else {
    ...
    for (const shift of userShifts) {
      ...
      const shiftOff = rosterDayOffUserIds.has(userId);
      if (shiftOff) block.classList.add("off");
      ...
      block.onclick = () => openRosterShiftDetail(shift);
...
async function refreshRosterWeekOverview() {
  ...
  if (shiftResult.error || offResult.error) {
    const error = shiftResult.error || offResult.error;
    $("rosterWeekOverviewGrid").innerHTML =
      `<div class="empty-state">${escapeHtml(error.message)}</div>`;
    return;
  }

  rosterWeekShifts = shiftResult.data || [];
  rosterWeekOffKeys = new Set(
    (offResult.data || []).map((row) => `${row.user_id}|${row.off_date}`)
  );
  renderRosterWeekOverview();
}
  ...
  for (const day of days) {
    ...
    if (!dayShifts.length) {
      ...
      for (const shift of dayShifts) {
        ...
        const off = rosterWeekOffKeys.has(`${shift.user_id}|${shift.shift_date}`);
        ...
        item.innerHTML = `
          ...
          <small>${escapeHtml(shift.assignment || "Fresh Produce")}${off ? " · OFF" : ""}</small>
        `;

        item.onclick = () => openRosterShiftDetail(shift);
...
function isRosterShiftOff(shift) {
  if (!shift) return false;
  if (
    shift.shift_date === rosterSelectedDate
    && rosterDayOffUserIds.has(shift.user_id)
  ) {
    return true;
  }
  return rosterWeekOffKeys.has(`${shift.user_id}|${shift.shift_date}`);
}

function openRosterShiftDetail(shift) {
  if (!shift) return;
  selectedRosterShiftDetail = shift;

  const off = isRosterShiftOff(shift);
  $("rosterShiftDetailName").textContent =
    rosterProfileMap.get(shift.user_id) || "User";
  $("rosterShiftDetailDate").textContent = formatRosterDate(shift.shift_date);
  $("rosterShiftDetailTime").textContent =
    `${formatRosterTime(shift.start_time)}–${formatRosterTime(shift.end_time)}`;
  $("rosterShiftDetailAssignment").textContent =
    shift.assignment || "Fresh Produce";
  $("rosterShiftDetailNotes").textContent = shift.notes || "No notes";

  const status = $("rosterShiftDetailStatus");
  status.textContent = off ? "OFF" : "Rostered";
  status.className = off ? "my-shift-state off" : "my-shift-state rostered";

  const ownerActions = $("rosterShiftDetailOwnerActions");
  ownerActions.classList.toggle("hidden", !isOwnerUser());

  const offButton = $("rosterShiftDetailOffBtn");
  offButton.textContent = off ? "Restore Shift" : "OFF";
  offButton.className = off ? "secondary" : "admin-danger";

  $("rosterShiftDetailModal").classList.remove("hidden");
  document.documentElement.classList.add("lookup-modal-open");
  document.body.classList.add("lookup-modal-open");
}

function closeRosterShiftDetail() {
  $("rosterShiftDetailModal")?.classList.add("hidden");
  selectedRosterShiftDetail = null;
  document.documentElement.classList.remove("lookup-modal-open");
  document.body.classList.remove("lookup-modal-open");
}

async function toggleSelectedRosterOff() {
  if (!isOwnerUser() || !selectedRosterShiftDetail || !currentUser) return;

  const shift = selectedRosterShiftDetail;
  const currentlyOff = isRosterShiftOff(shift);
  const button = $("rosterShiftDetailOffBtn");
  button.disabled = true;
  ...
  if (currentlyOff) {
    result = await supabase
      ...
      .eq("off_date", shift.shift_date);
  } else {
    result = await supabase
      .from("gcfr_roster_off_days")
      .upsert({
        user_id: shift.user_id,
        off_date: shift.shift_date,
        created_by: currentUser.id,
  ...
  const wasWeekView = rosterWeekOverviewOpen;
  closeRosterShiftDetail();
  showToast(currentlyOff ? "Shift restored." : "Marked OFF.");
  ...
  await myShiftController?.refresh();
}
...
async function runScanBarcodeType(barcode) {
  const { data, error } = await supabase.rpc(
    "gcfr_run_scan_barcode_type",
    {
      _barcode: String(barcode || "").trim(),
    },
  );
...
async function handleScannedBarcode(barcode) {
  ...
  const barcodeType = await runScanBarcodeType(barcode);

  if (barcodeType === "factory_barcode") {
    closeBarcodeLinkPanel();
    setScanStatus("Factory Code ignored · use Stock > Backstock.");
    showToast("Factory Code is for Backstock and cannot be added to a Run List.", 4200);
    return;
  }
...
function showAdminPricingView() {
  ...
  $("adminOwnerDocumentsView")?.classList.add("hidden");
  $("adminFreshResetView")?.classList.add("hidden");
...
$("openOwnerDocumentsBtn")?.addEventListener("click", async () => {
  if (!isOwnerUser()) return;
  stopAdminBarcodeScanner();
  $("adminHomeView")?.classList.add("hidden");
  $("adminBarcodeDetailView")?.classList.add("hidden");
  ...
  $("adminPricingView")?.classList.add("hidden");
  $("adminFreshResetView")?.classList.add("hidden");
  $("adminOwnerDocumentsView")?.classList.remove("hidden");
  $("screenTitle").textContent = "Documents";
  await freshResetController?.loadDocuments?.();
});

$("ownerDocumentsBackBtn")?.addEventListener("click", () => {
  freshResetController?.closeDocumentPreview?.();
  showAdminHomeView();
});

$("openFreshResetBtn")?.addEventListener("click", async () => {
  if (!isOwnerUser()) return;
  stopAdminBarcodeScanner();
  $("adminHomeView")?.classList.add("hidden");
  $("adminBarcodeDetailView")?.classList.add("hidden");
  ...
  $("adminPricingView")?.classList.add("hidden");
  $("adminOwnerDocumentsView")?.classList.add("hidden");
  $("adminFreshResetView")?.classList.remove("hidden");
  $("screenTitle").textContent = "Fresh Produce Reset";
  freshResetController?.renderReset?.();
  await freshResetController?.loadProgress?.();
});

$("freshResetBackBtn")?.addEventListener("click", showAdminHomeView);
