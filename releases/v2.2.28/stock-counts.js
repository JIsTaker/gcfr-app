...
export function initStockCounts({supabase, getUser, toast, onAreaChange, onBackstockSaved, onBackstockCancelled, renderBarcode, openStock}) {
  ...
  host.innerHTML = `
    ...
    <section class="card count-results">
      <div class="stock-section-head count-results-head">
        <div>
          <h3>Counted stock</h3>
          <small id="countSummary" class="count-summary">No stock scanned yet</small>
        </div>
        <input id="countDate" type="date" aria-label="Stock count date" value="${state.date}">
      </div>
      <div id="countLoadStatus" class="count-load-status" role="status"></div>
      <div id="countGroups"></div>
    </section>`;
  const q = (id) => document.getElementById(id);
  const areaName = (area) => area === 'backstock' ? 'Backstock' : 'Shopfloor';
  const setCameraPopup = (enabled) => q('countEditor')?.classList.toggle('count-editor-camera-popup', !!enabled);

  function discardPendingEntry({ notify = false } = {}) {
    const hadPending = !!state.selection || !!state.draftDetails || !q('countEditor')?.classList.contains('hidden');
    state.selection = null;
    state.editing = null;
    state.pendingId = null;
    state.draftDetails = null;
    setCameraPopup(false);
    const editor = q('countEditor');
    if (editor) {
      editor.innerHTML = '';
      editor.classList.add('hidden');
    }
    if (notify && hadPending) onBackstockCancelled?.();
    return hadPending;
  }
  ...
  q('countDate').onchange = () => {
    ...
    setCameraPopup(false);q('countEditor').classList.add('hidden');state.rows=[];renderRows();void load();
  };
  ...
  function renderRows(target=q('countGroups'),rows=state.rows,options={}) {
    ...
    const emptyText = options.emptyText || "No stock scanned yet.";
    ...
    }).join('') || `<div class="empty-state count-empty-state">${esc(emptyText)}</div>`;

    if (target === q('countGroups')) {
      const summary = q('countSummary');
      if (summary) {
        const backstockGroups = groupList.filter(group => group.area === 'backstock').length;
        const shopfloorGroups = groupList.filter(group => group.area === 'shopfloor').length;
        const parts = [];
        if (backstockGroups) parts.push(`${backstockGroups} backstock`);
        if (shopfloorGroups) parts.push(`${shopfloorGroups} shopfloor`);
        summary.textContent = parts.length ? parts.join(' · ') : 'No stock scanned yet';
      }
    }
  ...
  function renderBackstockQuickEntry(saved = null) {
    ...
    const scannerOpen = !document.getElementById('stockScannerWrap')?.classList.contains('hidden');
    setCameraPopup(scannerOpen);

    q('countEditor').innerHTML = `
      <div class="backstock-quick-entry">
        <button id="backstockQuickClose" class="backstock-quick-close" type="button" aria-label="Close quantity entry">×</button>
        ...
        <form id="backstockQuickForm" class="backstock-quick-form" novalidate>
          ...
          <button id="backstockQuickNext" class="primary" type="submit">Save</button>
        </form>
      </div>
    `;
    ...
    input.oninput = sync;
    q('backstockQuickClose').onclick = () => discardPendingEntry({ notify: true });
  ...
  function renderEditor(saved = null) {
    ...
    q('countCancel').onclick=()=>{if(!state.busy){state.selection=null;state.editing=null;state.pendingId=null;state.draftDetails=null;setCameraPopup(false);q('countEditor').classList.add('hidden');}};
    update();
  }
  ...
  async function save(details) {
    ...
    try{
      ...
      const totalBackstockPackages = details.area==='backstock'
        ? Math.round(state.rows
            .filter(item=>item.area==='backstock'&&backstockKey(item)===backstockKey(row))
            .reduce((sum,item)=>sum+backstockPackageCount(item),0))
        : 0;
      state.pendingId=null;state.editing=null;state.selection=null;state.draftDetails=null;setCameraPopup(false);q('countEditor').classList.add('hidden');renderRows();
      if(edit)toast('Count updated.');
      else if(details.area!=='backstock')toast('Added to calculator.');
      if(!edit&&details.area==='backstock')onBackstockSaved?.(row,totalBackstockPackages);
  ...
  function renderBackstockList(target) {
    ...
    renderRows(
      ...
      state.rows.filter(row=>row.area==='backstock'),
      {
        liveView:true,
        showTickets:false,
        emptyText:'No Backstock scanned yet. Scan a Factory Code to add stock.',
      },
    );
  }
  ...
  function reset(){++state.epoch;state.rows=[];state.selection=null;state.editing=null;state.pendingId=null;state.loadedUser=null;state.draftDetails=null;state.date=today();q('countDate').value=state.date;setCameraPopup(false);q('countEditor').classList.add('hidden');q('countLoadStatus').textContent='';document.getElementById('stockCountHistory')?.replaceChildren();renderRows();}
  return {
    select,
    load,
    loadHistory,
