...
async function startScanner() {
  if (!window.Html5Qrcode) {
    showToast("Scanner library failed to load. Use manual code entry.");
    return;
  }
  ...
  try {
    ...
    showToast("Camera could not start. Check camera permission.");
  }
}
...
function renderQuickTemplates() {
  ...
  for (const title of QUICK_CHECKLIST_TEMPLATES) {
    ...
    label.className = "quick-template-option";
...
document.querySelectorAll("#quickPeriodButtons button[data-period]").forEach((button) => {
  button.addEventListener("click", () => {
    quickAddPeriod = button.dataset.period;
    document
      .querySelectorAll("#quickPeriodButtons button[data-period]")
...
$("quickAddSelectedBtn").onclick = async () => {
  ...
  const selected = [...$("quickTemplateList").querySelectorAll('input[type="checkbox"]:checked')]
    .map((el) => el.value);
  ...
  const rows = selected.map((title) => ({
    ...
    title,
    time_period: quickAddPeriod,
    created_by: currentUser.id,
  }));

  const { error } = await supabase.from("checklist_tasks").insert(rows);
  ...
  $("quickAddPanel").classList.add("hidden");
  showToast(`${selected.length} item${selected.length === 1 ? "" : "s"} added to ${periodLabel(quickAddPeriod)}.`);
  await refreshChecklist();
};
