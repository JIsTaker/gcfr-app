...
export function initGcfrV2Stock({
  ...
  async function loadProfileAndPackages() {
  ...
  function renderDatabaseUnavailable(error) {
    const message = /gcfr_stock_profiles|gcfr_factory_packages|schema cache|does not exist/i
      .test(error?.message || "")
      ? "V2 stock database has not been applied yet."
      : (error?.message || "Unable to load stock data.");

    q("stockProfileSummary").innerHTML =
      `<div class="stock-warning">${escapeHtml(message)}</div>`;
  ...
  function renderSelectedProduct() {
    ...
    q("stockManagerBadge").classList.toggle("hidden", !manager);
    q("stockSetupBtn").classList.toggle("hidden", !manager);
    q("stockAddPackageBtn").classList.toggle("hidden", !manager || !profile);

    if (!profile) {
      const inferred = inferProfileFromName(state.selectedProduct.name);
      const hint = inferred.stock_type
        ? `Suggested: ${inferred.stock_type === "each" ? "1 Each" : "Approx"}${inferred.default_unit_weight_g ? ` · ${inferred.default_unit_weight_g}g` : ""}`
        : "Type not set";

      q("stockProfileSummary").innerHTML = `
        <div class="stock-profile-empty">
          <strong>Stock type not registered</strong>
      ...
      q("stockPackageList").innerHTML =
        '<div class="empty-state">Set the product type before adding package specifications.</div>';

      if (manager) openProfileSetup(false);
      else q("stockSetupPanel").classList.add("hidden");
    ...
    const typeLabel = profile.stock_type === "each" ? "1 Each" : "Approx";
    const unitWeight = cleanNumber(profile.default_unit_weight_g);

    q("stockProfileSummary").innerHTML = `
      <div class="stock-profile-ready">
        <span class="stock-type-pill">${typeLabel}</span>
        <div>
          <strong>${escapeHtml(state.selectedProduct.name)}</strong>
          <small>${profile.stock_type === "approx"
            ? (unitWeight ? `Default unit weight ${unitWeight}g` : "Weight entered when required")
            : "Counted by each"}</small>
        </div>
      </div>
    `;
  ...
  function renderPackages() {
    ...
    for (const row of state.packages) {
      ...
      option.value = row.id;
      option.textContent = `${packageTitle(row)} · ${packageDetail(row)}`;
  ...
  function renderCalculator() {
    ...
    q("stockPackageSelectedDetail").textContent = packageDetail(pack);

    if (!each) {
      const defaultWeight = cleanNumber(state.profile.default_unit_weight_g);
      if (!q("stockDisplayUnitWeightG").value && defaultWeight) {
        q("stockDisplayUnitWeightG").value = String(defaultWeight);
      }
  ...
  function openUnknownBarcode(barcode) {
    ...
    q("stockUnknownMessage").textContent = canManage()
      ? "This is not a recognised Store Ticket. Choose what this barcode represents."
      : "Unknown barcode. Joey, Troy J or Alex S must register this code.";
  ...
  async function chooseUnknownType(mode) {
    ...
    if (mode === "selling") {
      if (state.selectedProduct) {
        ...
        q("stockUnknownMessage").textContent =
          "Selling / Product Code selected. Search and open the product this code belongs to.";
    ...
    if (state.selectedProduct) {
      if (!state.profile) {
        openProfileSetup();
        q("stockUnknownMessage").textContent =
          "Factory Code selected. Set 1 Each or Approx first.";
      } else {
        openPackageEditor({ factory_barcode: state.pendingBarcode });
      }
    } else {
      q("stockUnknownMessage").textContent =
        "Factory Code selected. Search and open the product for this package.";
      q("stockProductSearch").focus();
    }
  }
  ...
  async function savePendingSellingBarcode() {
    ...
    closeUnknownBarcode();
    showToast(`Selling / Product Code linked to ${state.selectedProduct.name}.`);
  }
  ...
  function onScreenChange(screen) {
    ...
    q("stockManagerBadge")?.classList.toggle("hidden", !canManage());
  ...
  q("stockUnknownSellingBtn").onclick = () => chooseUnknownType("selling");
  q("stockUnknownFactoryBtn").onclick = () => chooseUnknownType("factory");
