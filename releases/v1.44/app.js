...
const NOTIFICATION_SETTINGS_KEY = "gcfr_notification_settings_v1";
...
let workflowNotificationChannel = null;

let notificationAudioContext = null;
let notificationSettings = loadNotificationSettings();
let webPushSubscription = null;
let webPushPermissionAttempted = false;
let checklistCreateAlertTimer = null;
let checklistCreateAlertCount = 0;
let checklistCreateAlertTitles = [];
...
function loadNotificationSettings() {
  try {
    ...
    return {
      ...
      sound: stored.sound ?? DEFAULT_NOTIFICATION_SETTINGS.sound,
      vibration: stored.vibration ?? DEFAULT_NOTIFICATION_SETTINGS.vibration,
      volume: Number.isFinite(Number(stored.volume))
        ? Math.max(0, Math.min(100, Number(stored.volume)))
        : DEFAULT_NOTIFICATION_SETTINGS.volume,
    };
  } catch {
...
function playNotificationSound() {
  ...
  const volume = Math.max(0, Math.min(100, Number(notificationSettings.volume) || 0));
  if (!volume) return;
  ...
  try {
    ...
    master.gain.exponentialRampToValueAtTime(
      Math.max(0.001, 0.18 * (volume / 100)),
      now + 0.015,
    );
    master.gain.exponentialRampToValueAtTime(0.0001, now + 0.42);
    master.connect(notificationAudioContext.destination);

    const first = notificationAudioContext.createOscillator();
    first.type = "sine";
    ...
    const second = notificationAudioContext.createOscillator();
    second.type = "sine";
...
function vibrateNotification() {
  if (!notificationSettings.enabled || !notificationSettings.vibration) return;
...
function rememberNotificationKey(key, ttl = 120000) {
  const now = Date.now();

  for (const [existingKey, expiresAt] of recentNotificationKeys) {
    if (expiresAt <= now) recentNotificationKeys.delete(existingKey);
  }

  if (recentNotificationKeys.has(key)) return false;

  recentNotificationKeys.set(key, now + ttl);
  return true;
}
...
async function showSystemNotification(title, body) {
  ...
  try {
    ...
    await registration.showNotification(title, {
      ...
      renotify: true,
      silent: false,
      vibrate: notificationSettings.vibration ? [160, 80, 160] : undefined,
      data: {
        url: "../../",
      },
    });
...
async function fireGcfrNotification({
  key,
  title,
  body,
  showToastToo = true,
  ...
  if (key && !rememberNotificationKey(key)) return;

  const isVisible = document.visibilityState === "visible";

  if (showToastToo && isVisible) {
    showToast(`${title}: ${body}`, 3400);
  }
...
function queueChecklistCreatedNotification(payload) {
  const row = payload.new || {};
  if (!row.id || row.created_by === currentUser?.id) return;

  checklistCreateAlertCount += 1;
  if (row.title) checklistCreateAlertTitles.push(row.title);

  clearTimeout(checklistCreateAlertTimer);
  checklistCreateAlertTimer = setTimeout(async () => {
    const count = checklistCreateAlertCount;
    const titles = [...checklistCreateAlertTitles];

    checklistCreateAlertCount = 0;
    checklistCreateAlertTitles = [];

    const sample = titles.slice(0, 2).join(", ");
    const extra = count > 2 ? ` +${count - 2}` : "";

    await fireGcfrNotification({
      key: `checklist-create-${Date.now() >> 10}-${count}-${sample}`,
      title: "Checklist updated",
      body: count === 1
        ? (sample || "A checklist item was added.")
        : `${count} checklist items added${sample ? ` — ${sample}${extra}` : ""}.`,
    });
  }, 650);
}

function handleRunListRealtime(payload) {
  if (payload.eventType !== "INSERT") return;

  const row = payload.new || {};
  if (!row.id || row.created_by === currentUser?.id) return;

  fireGcfrNotification({
    key: `run-list-insert-${row.id}`,
    title: "New Stock Run List",
    body: `Run List #${row.id} was created.`,
  });
}

function handleRunItemRealtime(payload) {
  if (payload.eventType !== "UPDATE") return;

  const row = payload.new || {};
  if (!row.id || row.completed_by === currentUser?.id) return;

  if (row.status === "done") {
    fireGcfrNotification({
      key: `run-item-done-${row.id}`,
      title: "Stock Run completed",
      body: row.manual_name || row.product_code
        ? `${row.manual_name || row.product_code} was completed.`
        : "A Stock Run item was completed.",
    });
  } else if (row.status === "out_of_stock") {
    fireGcfrNotification({
      key: `run-item-oos-${row.id}`,
      title: "Out of stock",
      body: row.manual_name || row.product_code
        ? `${row.manual_name || row.product_code} was marked Out of stock.`
        : "A Stock Run item was marked Out of stock.",
    });
  }
}

function handleChecklistRealtime(payload) {
  const row = payload.new || {};

  if (payload.eventType === "INSERT") {
    queueChecklistCreatedNotification(payload);
    return;
  }
  ...
  if (
    ...
    && row.id
    && row.status === "done"
    && row.completed_by !== currentUser?.id
  ) {
    fireGcfrNotification({
      key: `checklist-done-${row.id}`,
      title: "Checklist completed",
      body: row.title
        ? `${row.title} was completed.`
...
function handleWorkflowBroadcast(payload = {}) {
  if (!currentUser || payload.actor_id === currentUser.id) return;

  const eventType = payload.event_type;

  if (eventType === "run_created") {
    fireGcfrNotification({
      key: workflowNotificationKey(payload),
      title: "New Stock Run List",
      body: `Run List #${payload.run_id} was created.`,
    });
    return;
  }
  ...
  if (eventType === "run_done") {
    fireGcfrNotification({
      ...
      title: "Stock Run completed",
      body: payload.label
        ? `${payload.label} was completed.`
        : "A Stock Run item was completed.",
    });
    return;
  }

  if (eventType === "run_out_of_stock") {
    fireGcfrNotification({
      key: workflowNotificationKey(payload),
      title: "Out of stock",
      body: payload.label
        ? `${payload.label} was marked Out of stock.`
        : "A Stock Run item was marked Out of stock.",
    });
    return;
  }
  ...
  if (eventType === "checklist_created") {
    ...
    fireGcfrNotification({
      ...
      title: "Checklist updated",
      body: count === 1
        ? `${payload.title || "A checklist item"} was added.`
        : `${count} checklist items were added.`,
    });
    return;
  }

  if (eventType === "checklist_done") {
    fireGcfrNotification({
      key: workflowNotificationKey(payload),
      title: "Checklist completed",
      body: `${payload.title || "A checklist item"} was completed.`,
    });
  }
}

async function sendWorkflowBroadcast(eventType, payload = {}) {
  ...
  if (workflowNotificationChannel) {
    try {
      const response = await workflowNotificationChannel.send({
        type: "broadcast",
        event: "workflow_notification",
        payload: message,
      });

      realtimeOk = response === "ok";
    } catch (error) {
      console.warn("Workflow notification broadcast failed:", error);
    }
  }

  const pushOk = await sendWorkflowPush(eventType, payload, eventId);
...
function renderNotificationSettings() {
  ...
  const volumeValue = $("notificationVolumeValue");

  if (!enabled || !sound || !vibration || !volume || !volumeValue) return;

  enabled.checked = !!notificationSettings.enabled;
  sound.checked = !!notificationSettings.sound;
  vibration.checked = !!notificationSettings.vibration;
  volume.value = String(notificationSettings.volume);
  volumeValue.textContent = `${notificationSettings.volume}%`;

  sound.disabled = !notificationSettings.enabled;
  vibration.disabled = !notificationSettings.enabled;
  volume.disabled = !notificationSettings.enabled || !notificationSettings.sound;
}

async function ensureNotificationPermissionIfPossible() {
  if (!("Notification" in window)) return;

  // No separate "Blocked / Enable" state in GCFR.
  // The single Notifications toggle controls GCFR alerts.
  // If browser permission has never been decided, ask once when the user turns it on.
...
async function callPushFunction(action, payload = {}) {
  const { data, error } = await supabase.functions.invoke("gcfr-push", {
    body: {
      action,
      ...payload,
    },
  ...
  if (error) {
    throw new Error(error.message || "Push request failed.");
  }
...
async function syncWebPushSubscription() {
  ...
  await callPushFunction("subscribe", {
    ...
    preferences: {
      ...
      volume: Number(notificationSettings.volume) || 0,
    },
  });
...
async function enableWebPush({
  ...
  if (!currentUser || !notificationSettings.enabled) return false;

  if (!webPushSupported()) {
    if (!quiet) {
      showToast("Background notifications are not supported on this device/browser.");
    }
    return false;
  }

  if (Notification.permission === "default" && requestPermission) {
    webPushPermissionAttempted = true;

    const permission = await Notification.requestPermission();
    if (permission !== "granted") {
      if (!quiet) showToast("Notification permission was not granted.");
      return false;
    }
  }
  ...
  try {
    ...
    if (!subscription) {
      const { publicKey } = await callPushFunction("public_key");
    ...
    if (!quiet) {
      showToast(error.message || "Background notification setup failed.");
    }
...
async function disableWebPush() {
  ...
  try {
    ...
    try {
      ...
      console.warn("Could not remove server push subscription:", error);
    }

    await subscription.unsubscribe();
    webPushSubscription = null;
  } catch (error) {
    console.warn("Could not disable Web Push:", error);
  }
}

async function sendWorkflowPush(eventType, payload = {}, eventId = "") {
  if (!currentUser) return false;

  try {
    await callPushFunction("send_event", {
      eventType,
      eventId,
      payload,
    });
    ...
    console.warn("Workflow Web Push failed:", error);
    return false;
  }
}

function workflowNotificationKey(payload = {}) {
  if (payload.event_id) return `workflow-${payload.event_id}`;
...
navigator.serviceWorker?.addEventListener("message", (event) => {
  if (event.data?.type !== "GCFR_PUSH_FOREGROUND") return;

  const payload = event.data.payload || {};

  fireGcfrNotification({
    key: workflowNotificationKey(payload),
    title: payload.title || "GCFR",
...
async function enterApp() {
  ...
  if (
    ...
    enableWebPush({ requestPermission: false, quiet: true });
  }
}
...
async function leaveApp() {
  ...
  realtimeChannels = [];
  workflowNotificationChannel = null;
  currentUser = null;
  currentProfile = null;
...
function subscribeRealtime() {
  ...
  const checklistChannel = supabase
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "checklist_comments" }, () => refreshChecklistPreservingScroll())
    .on("postgres_changes", { event: "*", schema: "public", table: "checklist_shift_closures" }, () => refreshChecklistPreservingScroll())
    .subscribe();

  workflowNotificationChannel = supabase
    .channel("gcfr-workflow-notifications", {
      config: {
        broadcast: {
          self: false,
          ack: true,
    ...
    .on("broadcast", { event: "workflow_notification" }, ({ payload }) => {
      handleWorkflowBroadcast(payload);
    })
    .subscribe();

  realtimeChannels.push(runChannel, checklistChannel, workflowNotificationChannel);
}
...
$("submitRunBtn").onclick = async () => {
  ...
  try {
    ...
    await sendWorkflowBroadcast("run_created", {
      run_id: data,
    });
...
async function finishItem(id, status) {
  ...
  const { data: itemBefore } = await supabase
    .from("run_items")
    .select("product_code, manual_name, products(name)")
    .eq("id", id)
    .maybeSingle();

  if (itemBefore) {
    label =
      itemBefore.manual_name
      || itemBefore.products?.name
      || itemBefore.product_code
      || "";
  }
  ...
  await sendWorkflowBroadcast(
    status === "done" ? "run_done" : "run_out_of_stock",
    {
      item_id: id,
      label,
    },
...
$("quickAddSelectedBtn").onclick = async () => {
  ...
  await sendWorkflowBroadcast("checklist_created", {
    count: rows.length,
    title: rows.length === 1 ? rows[0].title : "",
  });
...
function renderChecklistTask(task, comments = [], commentAuthorNames = new Map(), closure = null) {
  ...
  if (isSelectingForEdit && (task.status === "done" || task.status === "carried")) {
    ...
    checkbox.onchange = async () => {
      ...
      if (done) {
        await sendWorkflowBroadcast("checklist_done", {
          task_id: task.id,
          title: task.title,
        });
      }
