...
export function initGcfrV2Stock({
  ...
  const counts = initStockCounts({
    ...
    onBackstockSaved() {
      setTimeout(() => {
        ...
        if (!q('stockCartonPickerModal')?.classList.contains('hidden')) return;
        void startScanner();
      }, 100);
    },
  });
  ...
  function closeCartonPicker() {
    state.pendingCartonBarcode = "";
    q("stockCartonPickerModal")?.classList.add("hidden");
    document.documentElement.classList.remove("lookup-modal-open");
    document.body.classList.remove("lookup-modal-open");
  }

  function openCartonPicker(barcode = "") {
    const modal = q("stockCartonPickerModal");
    const list = q("stockCartonPickerList");
    if (!modal || !list || !state.selectedProduct) return false;

    state.selectedPackageId = "";
    state.pendingCartonBarcode = barcode || "";
    renderOperationalPackageOptions();
    renderCalculator();

    q("stockCartonPickerProduct").textContent =
      `${state.selectedProduct.name} · ${state.selectedProduct.code}`;

    list.innerHTML = "";

    if (!state.packages.length) {
      list.innerHTML =
        '<div class="empty-state">No carton specifications are registered for this product.</div>';
    } else {
      for (const row of state.packages) {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "stock-carton-choice";
        button.dataset.cartonId = String(row.id);
        button.innerHTML = `
          <span>
            <strong>${escapeHtml(packageTitle(row))}</strong>
            <small>Factory Code ${escapeHtml(row.factory_barcode || "-")}</small>
          </span>
          <b>${escapeHtml(packageDetail(row))}</b>
        `;
        button.addEventListener("click", async () => {
          const barcodeForCount = state.pendingCartonBarcode;
          state.selectedPackageId = String(row.id);
          state.pendingCartonBarcode = "";
          renderOperationalPackageOptions();
          renderCalculator();
      ...
      if (factory) {
        ...
        if (product) {
          await selectProduct(product, {
            barcode,
            deferCount: true,
            deferPackageChoice: true,
          });
          openCartonPicker(barcode);
          return;
        }
      }
  ...
  q("stockCartonPickerCloseBtn")?.addEventListener("click", closeCartonPicker);
  q("stockCartonPickerBackdrop")?.addEventListener("click", closeCartonPicker);
