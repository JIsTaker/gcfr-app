...
export function initStockCounts({supabase, getUser, toast, onAreaChange, onBackstockSaved, renderBarcode, openStock}) {
  ...
  function renderRows(target=q('countGroups'),rows=state.rows) {
    ...
    target.innerHTML = groupList.map((group,index)=>{
      ...
      if (group.area === 'backstock') {
        ...
        return `<section class="count-group count-group-simple count-backstock-simple">
          ...
          <button type="button" class="text-button" data-ticket-code="${esc(group.code)}">Show product ticket</button><div class="count-ticket hidden"></div>
        </section>`;
      }
    ...
    const currentView = target === q('countGroups');
    ...
    target.querySelectorAll('[data-backstock-delete]').forEach(button=>button.onclick=async()=>{
      const group=groupList[Number(button.dataset.backstockDelete)];if(group)await setBackstockGroupQuantity(group.rows,0,currentView);
    });
    target.querySelectorAll('[data-delete-count]').forEach(button=>button.onclick=async()=>{
      const row=rows.find(item=>item.id===button.dataset.deleteCount);if(row)await deleteCountRow(row,currentView);
    });
  ...
  async function save(details) {
    ...
    try{
      ...
      state.pendingId=null;state.editing=null;state.selection=null;state.draftDetails=null;q('countEditor').classList.add('hidden');renderRows();toast(edit?'Count updated.':details.area==='backstock'?'Backstock +1':'Added to calculator.');if(!edit&&details.area==='backstock')onBackstockSaved?.(row);
  ...
  async function select(selection,{auto=false}={}) {
    ...
    if(auto&&state.area==='backstock'&&selection.pack&&selection.profile){
      ...
      if(number(amount)>0&&(each||selection.pack.approx_weight_mode==='fixed')){
        await save({area:'backstock',unit:each?'each':'kg',kind:'new',mode:each?'each':'fixed',perPackage:amount,packages:1,packageId:String(selection.pack.id||''),packageLabel:String(selection.pack.package_label||'Carton'),factoryBarcode:String(selection.pack.factory_barcode||selection.barcode||'')});
        return;
      }
    }
    renderEditor();
  ...
  return {select,load,loadHistory,reset,area:()=>state.area,isBusy:()=>state.busy,snapshotPrivateWork,restorePrivateWork};
}
