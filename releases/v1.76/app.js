...
const DAILY_MANAGER_USERS = new Map([
...
function knownUserDisplayName(userId, nameMap = null) {
  ...
  return DAILY_MANAGER_USERS.get(userId) || nameMap?.get(userId) || "User";
}
...
async function enterApp() {
  ...
  $("accountRole").textContent =
    currentUser.id === ADMIN_USER_ID
      ? "admin"
      : (DAILY_MANAGER_USERS.has(currentUser.id) ? "daily manager" : "user");

  const isAdmin = currentUser.id === ADMIN_USER_ID;
  const canManageProducts = canManageDailyOperations();
...
$("productCodeLookupSearch").addEventListener("input", () => {
  ...
  if (!query) {
    $("productCodeLookupResults").innerHTML =
      `<div class="empty-state">Enter a product name, Product Code, or Ticket Barcode.</div>`;
    return;
  }
...
function renderProductCodeLookupResults(rows) {
  const results = $("productCodeLookupResults");
  results.innerHTML = "";
  ...
  for (const row of rows) {
    ...
    const ticketBarcodeText = row.ticketBarcodes.length
      ? row.ticketBarcodes.join(" / ")
      : "Not registered";

    button.innerHTML = `
      <div class="lookup-result-name">${escapeHtml(row.name || "Unknown product")}</div>

      <div class="lookup-data-row">
      ...
      <div class="lookup-data-row">
        ...
        <strong class="${row.ticketBarcodes.length ? "" : "lookup-missing"}">
          ${escapeHtml(ticketBarcodeText)}
        </strong>
      </div>

      <div class="lookup-open-hint">Tap to show barcode</div>
    `;
...
function openProductCodeLookupDetail(row) {
  ...
  const barcodes = (row.ticketBarcodes || [])
...
$("submitRunBtn").onclick = async () => {
  ...
  try {
    ...
    if (submittedRun) {
      const numberMap = await buildDailyRunNumberMap([submittedRun]);
      submittedRunNumber = numberMap.get(submittedRun.id) || 1;
    }

    showToast(`Run #${submittedRunNumber} submitted.`);
...
$("barcodeManualRegisterLaunchBtn").onclick = () => {
  if (!pendingUnknownBarcode || !canManageDailyOperations()) return;
...
function showUnknownBarcode(barcode) {
  ...
  const isAdmin = currentUser?.id === ADMIN_USER_ID;
  const canRegister = canManageDailyOperations();
  ...
  $("barcodeLinkMessage").textContent = isAdmin
    ...
    : canRegister
      ? "This barcode is not linked. Use Manual Register if the product needs to be added or linked."
...
$("barcodeManualRegisterForm").onsubmit = async (event) => {
  ...
  if (!pendingUnknownBarcode || !canManageDailyOperations()) {
    showToast("You do not have permission to register product data.");
    return;
  }
...
async function refreshRuns() {
  ...
  // Explicit scanned ticket barcodes only.
  // Product Code is separate and is never inferred as a barcode.
  ...
  if (runProductCodes.length) {
    ...
    if (!barcodeError) {
      for (const row of barcodeRows || []) {
        ...
        // Show only numeric 7- or 8-digit ticket barcodes.
        if (!/^\d{7,8}$/.test(barcode)) continue;
  ...
  currentVisibleRunIds = visibleRuns.map((run) => run.id);
  ...
  if (!visibleRuns.length) {
    resetRunBulkDelete();
    updateRunBulkDeleteControls();
    container.innerHTML = `<div class="empty-state">No run lists yet.</div>`;
    return;
  }

  updateRunBulkDeleteControls();

  const dailyNumberMap = await buildDailyRunNumberMap(visibleRuns);
  ...
  for (const run of visibleRuns) {
    ...
    if (runBulkDeleteMode && canManageDailyOperations()) {
      ...
      selectBox.dataset.runBulkId = String(run.id);
      selectBox.checked = runBulkDeleteIds.has(run.id);

      selectBox.onchange = () => {
        if (selectBox.checked) {
          runBulkDeleteIds.add(run.id);
        } else {
          runBulkDeleteIds.delete(run.id);
        }
    ...
    if (canManageDailyOperations() && !runBulkDeleteMode) {
      ...
      deleteRun.textContent = "Remove Run";
      deleteRun.onclick = async () => {
        if (!confirm(`Remove Run #${runNumber}?`)) return;

        const { data, error } = await removeRunListByPermission(run.id);

        if (error) return showToast(error.message);
        if (!data) return showToast("Run could not be removed.");

        showToast(
          isDelegateManager()
            ? `Run #${runNumber} moved to History. Final deletion is pending Joey approval.`
            : `Run #${runNumber} moved to History.`
        );
...
function renderChecklistTask(task, comments = [], commentAuthorNames = new Map(), closure = null) {
  ...
  if (canManageDailyOperations() && !checklistBulkDeleteMode) {
    ...
    if (pendingRemoval && isDelegateManager()) {
      del.textContent = "Pending";
      del.disabled = true;
    } else if (pendingRemoval && isOwnerUser()) {
      del.textContent = "Final Delete";
    } else {
      del.textContent = isDelegateManager() ? "Remove" : "Delete";
    }

    del.onclick = async () => {
      const actionText = isDelegateManager()
