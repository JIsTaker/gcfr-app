...
function formatLookupPrice(value, { perKg = false } = {}) {
  ...
  return perKg ? `${formatted} / 1kg` : formatted;
}
...
async function loadProductLookupPrice(row) {
  ...
  try {
    ...
    // Coles approx. produce is sold/displayed by kilogram.
    // Prefer the explicit comparable /kg value when Coles supplies it.
    const displayNow = comparablePerKg ?? now;
    const displayWas = was;
