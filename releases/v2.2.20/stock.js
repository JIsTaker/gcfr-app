...
export function initGcfrV2Stock({
  ...
  function showScannerProductFeedback({
    ...
    if (!card) return;
    if (q("stockScannerDetectedName")) q("stockScannerDetectedName").textContent = name || "-";
    if (q("stockScannerDetectedMeta")) {
      q("stockScannerDetectedMeta").textContent =
        `Product ${code || "-"} · Factory ${barcode || "-"}`;
    }
    if (q("stockScannerDetectedResult")) {
      q("stockScannerDetectedResult").textContent = result || "Detected";
    }
  ...
  const counts = initStockCounts({
    ...
    onBackstockSaved(row) {
      showScannerProductFeedback({
        ...
        result: `Backstock +${row?.details?.packages || 1}`,
      });
      setTimeout(() => {
      ...
      if (factory) {
        ...
        if (product) {
          showScannerProductFeedback({
            ...
            barcode,
            result:
              state.backstockScanEntryMode === "quantity"
                ? "Enter quantity"
                : "Product detected",
          });
