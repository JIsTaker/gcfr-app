...
function handleWorkflowBroadcast(payload = {}) {
  ...
  if (eventType === "run_created") {
    fireGcfrNotification({
      key: `run-list-insert-${payload.run_id}`,
      title: "New Stock Run List",
  ...
  if (eventType === "run_done") {
    fireGcfrNotification({
      key: `run-item-done-${payload.item_id}`,
      title: "Stock Run completed",
      body: payload.label
  ...
  if (eventType === "run_out_of_stock") {
    fireGcfrNotification({
      key: `run-item-oos-${payload.item_id}`,
      title: "Out of stock",
      body: payload.label
  ...
  if (eventType === "checklist_created") {
    ...
    fireGcfrNotification({
      key: `checklist-created-${payload.event_id}`,
      title: "Checklist updated",
      body: count === 1
  ...
  if (eventType === "checklist_done") {
    fireGcfrNotification({
      key: `checklist-done-${payload.task_id}`,
      title: "Checklist completed",
...
async function sendWorkflowBroadcast(eventType, payload = {}) {
  if (!currentUser || !workflowNotificationChannel) return false;
...
document.addEventListener(
  ...
  () => {
    ...
    ensureNotificationPermissionIfPossible();
  },
  { passive: true },
);
...
$("notificationEnabled").onchange = async () => {
  ...
  if (notificationSettings.enabled) {
    await unlockNotificationAudio();
    await ensureNotificationPermissionIfPossible();
  }
...
$("testNotificationBtn").onclick = async () => {
  ...
  await fireGcfrNotification({
    key: `test-${Date.now()}`,
    title: "GCFR test",
    body: "Notification settings are working.",
    showToastToo: true,
  });
};
